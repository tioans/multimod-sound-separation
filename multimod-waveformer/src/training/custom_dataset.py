...
class CVSoundScapesDataset(torch.utils.data.Dataset):  # type: ignore
    """
    Base class for CV Sound Scapes dataset
    """
    ...
    def _get_label_vector(self, labels):
        ...
        """
        vector = torch.zeros(len(CVSoundScapesDataset._labels))

        for label in labels:
            idx = CVSoundScapesDataset._labels.index(label)
...
def tensorboard_add_sample(writer, tag, sample, step, params):
    ...
    for b in range(m.shape[0]):
        ...
        for i in range(len(l[b, :])):
            if l[b, i] == 1:
                label.append(CVSoundScapesDataset._labels[i])
...
def tensorboard_add_metrics(writer, tag, metrics, label, step):
    ...
    label_names = [CVSoundScapesDataset._labels[torch.argmax(_)] for _ in label]
